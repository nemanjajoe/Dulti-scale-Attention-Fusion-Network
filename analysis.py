...
#   y_ = model(x_)
  ...
  bg_x = np.zeros((512,512,3),np.float32)
  bg_y = np.zeros((512,512,3),np.float32)
#   threshold = 0.
  for i in range(2, 3): # organs 0-9 0: background
   #   mask = y_[0][i].detach().numpy()
   #   mask[mask < threshold] = 0
   #   mask[mask >= threshold] = 1 
     mask_gt = label == i
     if len(np.unique(mask_gt)) == 2:
        count += 1
        print(count)
     fg = np.ones((512, 512, 3)) * colors[i]
